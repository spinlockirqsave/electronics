% Simulate forced oscillations in R load circuit.
%
% Params:
...
function [] = r_load_simulate(t, R, EMFm, omega_d)
    ...
    omega_d_ = omega_d;     % LC oscillator frequency
    ...
    for i = 1 : n
        ...
        % In R load we have from the Kirchhoff's loop rule for voltage:
        % EMF = v_R  where vR is voltage through the resistor
    ...
    plot_as_one(t, v_R_vec, 'v_R - voltage on resistor', i_R_vec, 'i_R - current through resistor');
    plot_as_sub(t, v_R_vec, 'v_R - voltage on resistor', i_R_vec, 'i_R - current through resistor');
